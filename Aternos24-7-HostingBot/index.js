    ...
    <html>
      <head>
        ...
        <style>
          ...
          .container { background: #1e293b; padding: 40px; border-radius: 20px; box-shadow: 0 0 50px rgba(45, 212, 191, 0.2); text-align: center; width: 400px; border: 1px solid #334155; transition: box-shadow 0.3s ease; }
          ...
          .btn-guide { display: inline-block; margin-top: 20px; padding: 12px 24px; background: #2dd4bf; color: #0f172a; text-decoration: none; border-radius: 8px; font-weight: bold; box-shadow: 0 0 15px rgba(45, 212, 191, 0.4); transition: transform 0.2s; }
          ...
          .connection-fill { height: 100%; width: 100%; background: #2dd4bf; animation: loading 2s infinite linear; transform-origin: 0% 50%; }
      ...
      <body>
        <div class="container" id="main-container">
          ...
          <div class="connection-bar"><div class="connection-fill" id="activity-bar"></div></div>
          <p style="color: #64748b; font-size: 12px; margin-top: 15px;">Live connection to Bot Process</p>
        </div>
        <script>
...
function createBot() {
  ...
  try {
    ...
    bot.once('spawn', () => {
      ...
      setTimeout(() => {
        if (bot && botState.connected) bot.chat('/gamerule sendCommandFeedback false');
      }, 3000);
...
function initializeModules(bot, mcData, defaultMove) {
  ...
  if (config.utils['periodic-rejoin'] && config.utils['periodic-rejoin'].enabled) periodicRejoin(bot);
  ...
  bot.on('health', () => {
    ...
    try {
      if (bot.food < 14) {
        ...
        if (food) {
          bot.equip(food, 'hand').then(() => bot.consume()).catch(e => console.log('[AutoEat] Error:', e.message));
        }
      }
    } catch (e) {
...
rl.on('line', (line) => {
  ...
  else if (trimmed === 'reconnect') { console.log('[Console] Manual reconnect requested'); bot.end(); }
...
process.on('SIGINT', () => { console.log('[System] Manual stop requested. Exiting...'); process.exit(0); });
